...
Процедура ПриНажатииОткрытьВСБИСПолучитьUID( Команда, ПараметрКоманды ) Экспорт 
	...
	ПроверкаОбмена = Saby_ВстраиваниеВСтандартныеФормы.ПроисходилЛиОбменДокументов(мСсылок.Источник);
...
Процедура ОткрытьВСБИС_ПолучитьURL( Источник, ПараметрКоманды ) Экспорт 
	...
	Попытка
		СсылкиНаДокументыВСБИС = Saby_ВстраиваниеВСтандартныеФормы.ПолучитьСсылкиНаДокументыВИнтеграции( Источник );
	Исключение
	...
	Иначе
		Если Не ОшибкаВыполнения Тогда
